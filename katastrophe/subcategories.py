...
def url_generator(url, page, category):
	...
	if len(words) == 1:
		link = 'https://kickasstorrents.to/usearch/' + words[0] + '%' + '20category%3' + 'A' + category_list[category]
	else:
		for i in xrange_(len(words)):
			if i == 0:
				link = 'https://kickasstorrents.to/usearch/' + words[i]
			else:
				link += '%20' + words[i]
...
def xxx_torrent(ssl):
    link = 'https://kickasstorrents.to/xxx/1/?field=time_add&sorder=desc'
    source_code = requests.get(link, verify = ssl)
    plain_text = source_code.text.encode('utf-8')
    soup = BeautifulSoup(plain_text, "lxml")
